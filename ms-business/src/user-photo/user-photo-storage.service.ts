...
export type UserPhotoStorageFile = {
  buffer: Buffer;
  originalname: string;
  mimetype: string;
  size: number;
...
export class UserPhotoStorageService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.bucket =
      this.configService.get<string>('SUPABASE_USER_BUCKET') ?? 'user-photo';
  }
  ...
  async upload(
    ...
    if (!this.supabaseUrl || !this.supabaseKey) {
      throw new BadRequestException(
        'Supabase Storage is not configured for user photos',
      );
    }
    ...
    const path = `users/${userId}/${Date.now()}-${randomUUID()}${extension}`;
    ...
    if (!response.ok) {
      ...
      throw new BadRequestException(
        `Could not upload user photo to Supabase Storage: ${detail}`,
      );
    }
  ...
  private extensionFromMime(file: UserPhotoStorageFile) {
