/**
 * Apply V7__address_admin_permissions.sql using ms-business/.env DB_URL.
...
(async () => {
  ...
  const sql = fs.readFileSync(
    path.join(__dirname, 'V7__address_admin_permissions.sql'),
    'utf8',
  );
  await c.query(sql);
  const n = await c.query(`
    select count(*)::int as n from security.permissions
    where url like '/address%'
  `);
  console.log('address permissions:', n.rows[0].n);
  await c.end();
  console.log('OK: V7 address admin permissions applied.');
})().catch((e) => {
  console.error(e);
  process.exit(1);
});
